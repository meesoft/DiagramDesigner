...
// Fix thread locale which seems to be wrong in Win7
procedure FixThreadLocale;
var
  Locale : LCID;
  OSVersionInfo: TOSVersionInfo;
begin
  OSVersionInfo.dwOSVersionInfoSize := SizeOf(OSVersionInfo);
  if GetVersionEx(OSVersionInfo) and (OSVersionInfo.dwMajorVersion>=5) then // Supported from Win2k
  begin
    Locale:=GetUserDefaultLCID;
    if Locale>$400 then SetThreadLocale(Locale);
  end;
end;
