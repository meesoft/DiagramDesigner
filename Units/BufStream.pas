...
uses
  {$IFDEF CLR} Borland.Win32.WinUtils, Borland.Delphi.SysUtils, {$ELSE} SysUtils, {$ENDIF}
  Streams;
...
type
 TBufferedStream = class (TFilterStream)
    private
...
implementation
 ...
 function OpenBufferedFile(Name: string; Mode: Integer): TBufferedStream;
 begin
   if Mode=fmRead then Result:=TBufferedStream.Create(DefaultBuffer,0,TFileStream.Create(Name,[fsRead,fsShareRead,fsShareWrite]))
