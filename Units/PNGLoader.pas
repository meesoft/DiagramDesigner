...
uses Windows, Classes, SysUtils, LinarBitmap, Streams, BufStream, Graphics, DelphiStream, MemUtils,
  CRC32Stream, Deflate, ColorMapper, Math, MemStream, FileMappedBitmap, MultiCoreProcessing;
...
resourcestring
  ...
  rsInvalidAlphaChannelFormat = 'Invalid alpha channel format or dimensions';
...
type
  ...
  TPNGLoader = class(TFileMappedBitmapLoader)
    public
      ...
      BitsPerPixel : Integer; // BPP for loaded image. Can be set to 1, 2 or 4 when saving 8 bit palette images to reduce size
      ScanLineFilter : Integer; // Default is slfAutoMinSum
      NoCompression : Boolean; // Turn off Deflate compression
...
// Do alpha blending to background color
procedure ApplyAlphaChannel(Image,Alpha: TLinearBitmap; BackgroundColor: TColor);
function CreateAlphaChannelFromColorKey(Image: TLinearBitmap; TransparentColor: Integer): TLinearBitmap;

implementation

uses Adler32, Syncobjs;
...
var
  ...
  Pix : PRGBRec;
begin
  Assert(Image.Width=Alpha.Width);
  Assert(Image.Height=Alpha.Height);
  Assert(Alpha.PixelFormat=pf8bit);

  Image.PixelFormat:=pf24bit;
  Pix:=Pointer(Image.Map);
  AlphaPix:=Pointer(Alpha.Map);
  for I:=1 to Alpha.Size do
  begin
    if AlphaPix^<>255 then
    begin
      Pix^.R:=(AlphaPix^*Pix^.R+TPALETTEENTRY(BackgroundColor).peRed*(255-AlphaPix^)) div 255;
      Pix^.G:=(AlphaPix^*Pix^.G+TPALETTEENTRY(BackgroundColor).peGreen*(255-AlphaPix^)) div 255;
      Pix^.B:=(AlphaPix^*Pix^.B+TPALETTEENTRY(BackgroundColor).peBlue*(255-AlphaPix^)) div 255;
    end;
    Inc(AlphaPix);
    Inc(Pix);
  end;
...
var
  ...
  Pix24 : PRGBRec;
begin
  Result:=TLinearBitmap.Create(Image.Width,Image.Height,pf8bit);
  Result.PixelFormat:=pf8bit;
  case Image.PixelFormat of
    pf8bit  : for Y:=0 to Image.Height-1 do
              begin
                Pix8:=Image.ScanLine[Y];
                DestPix:=Result.ScanLine[Y];
                for X:=1 to Image.Width do
                begin
                  if Pix8^=TransparentColor then DestPix^:=0
                  else DestPix^:=255;
                  Inc(DestPix);
                  Inc(Pix8);
                end;
    ...
    pf24bit : begin
                TransparentColor:=RGB2BGR(TransparentColor);
                for Y:=0 to Image.Height-1 do
                begin
                  Pix24:=Image.ScanLine[Y];
                  DestPix:=Result.ScanLine[Y];
                  for X:=1 to Image.Width do
                  begin
                    if PInteger(Pix24)^ and $ffffff=TransparentColor then DestPix^:=0
                    else DestPix^:=255;
                    Inc(DestPix);
                    Inc(Pix24);
                  end;
...
begin
  ...
  begin
    ...
    begin
      Got:=GetSwap4(TCRC32Stream(Next).CRC);
      Next.Read(CRC,4);
//{$MESSAGE WARN 'Disable CRC?'}    
      if CRC<>Got then raise Exception.Create(rsCRCError); // <<<<<<<<<<<<<<<<<<<<<<<<<<<
...
type
  ...
  TReadAheadData = record
                     NextFilter : Byte;
                     NextLine : PByteArray;
                     BytesPerPNGLine : Cardinal;
                     DeflateStream : TDeflateStream;
                     StartEvent, DoneEvent : TEvent;
                   end;
  PReadAheadData = ^TReadAheadData;

procedure ReadAhead(Thread: TSimpleThread; Data: Pointer);
begin
  with PReadAheadData(Data)^ do
  try
    repeat
      StartEvent.WaitFor(INFINITE);
      if DeflateStream=nil then Exit;
      DeflateStream.Read(NextFilter,1); // Read filter type
      DeflateStream.Read(NextLine^,BytesPerPNGLine); // Read line data
      DoneEvent.SetEvent;
    until False;
  except
    NextFilter:=255;
...
var
  ...
  var
    ...
    ReadAheadThread : TSimpleThread;
    ReadAheadData : TReadAheadData;
    ImageLine, PrevLine : PByteArray;
    Adam7PassesMap, Adam7PassesAlpha : TAdam7Planes;
    BPP, BytesPerPNGLine : Integer;
    Adler32 : TAdler32;
    ...
    try
      ...
      try
        ...
        begin
          ...
          if (LineCount_1<0) or (BytesPerPNGLine<=0) then Continue;

          ReadAheadData.BytesPerPNGLine:=BytesPerPNGLine;
          ReadAheadData.DeflateStream:=DeflateStream;
          ReadAheadData.StartEvent:=TEvent.Create(nil,False,False,'');
          ReadAheadData.DoneEvent:=TEvent.Create(nil,False,False,'');
          ReadAheadThread:=TSimpleThread.Create(ReadAhead,@ReadAheadData);
          GetMem(ReadAheadData.NextLine,BytesPerPNGLine);
          GetMem(ImageLine,BytesPerPNGLine);
          GetMem(PrevLine,BytesPerPNGLine);
          try
            ReadAheadData.StartEvent.SetEvent;
            ZeroMem(ImageLine^,BytesPerPNGLine);
            for Y:=0 to LineCount_1 do
            begin
              SwapDWords(ImageLine,PrevLine);

              //DeflateStream.Read(Filter,1); // Read filter type
              //DeflateStream.Read(ImageLine^,BytesPerPNGLine); // Read line data
              ReadAheadData.DoneEvent.WaitFor(INFINITE);
              Filter:=ReadAheadData.NextFilter;
              SwapDWords(ImageLine,ReadAheadData.NextLine);
              if Y=LineCount_1 then ReadAheadData.DeflateStream:=nil;
              ReadAheadData.StartEvent.SetEvent;
              ...
              case Filter of
                ...
                2 : for X:=0 to BytesPerPNGLine-1 do Inc(ImageLine^[X],PrevLine^[X]); // Up
                3 : begin // Avarage
                      for X:=0 to BPP-1 do Inc(ImageLine^[X],PrevLine^[X] div 2);
                      for X:=BPP to BytesPerPNGLine-1 do Inc(ImageLine^[X],(Word(ImageLine^[X-BPP])+PrevLine^[X]) div 2);
                    end;
                4 : begin // Paeth
                      for X:=0 to BPP-1 do Inc(ImageLine^[X],PaethPredictor(0,PrevLine^[X],0));
                      for X:=BPP to BytesPerPNGLine-1 do
                        Inc(ImageLine^[X],PaethPredictor(ImageLine^[X-BPP],PrevLine^[X],PrevLine^[X-BPP]));
                    end;
              else raise Exception.Create(rsInvalidPNGData);
              end;
              ...
              case IHDR.BitDepth of
                ...
                2  : for X:=0 to LineLength_1 do
                     begin
                       Pix^:=((SourcePix^ shr ((not X and 3) shl 1)) and $3);
                       if X and 3=3 then Inc(SourcePix);
                       Inc(Pix);
                     end;
                4  : for X:=0 to LineLength_1 do
                     begin
                       Pix^:=((SourcePix^ shr ((not X and 1) shl 2)) and $f);
              ...
              if Assigned(ProgressUpdate) and (LineCount_1>0) then ProgressUpdate(Y*100 div LineCount_1);
            end;
          finally
            FreeMem(PrevLine);
            FreeMem(ImageLine);
            FreeMem(ReadAheadData.NextLine);
            ReadAheadThread.Free;
            ReadAheadData.StartEvent.Free;
            ReadAheadData.DoneEvent.Free;
          end;
        end;
      ...
      if DWord(X)<>Adler32.CheckValue then raise Exception.Create(rsAdler32Error);
    finally
      IDATStream.Free;
    end;
  ...
  begin
    ...
    BitsPerPixel:=IHDR.BitDepth*Planes;
  end;
...
var
  ...
  var
    ...
    procedure AverageFilter(FilterLine: PByteArray);
    var X : DWord;
    begin
  ...
  begin
    ...
    try
      ...
      try
        ...
        Adler32.Init;
        FileMappedBitmap:=nil;
        if MustUpdateScanLine(Bitmap) then FileMappedBitmap:=TFileMappedBitmap(Bitmap);
        ZeroMem(ImageLine^,BytesPerLine);
        Pix:=@Bitmap.Map^;
        if Assigned(AlphaChannel) then AlphaPix:=@AlphaChannel.Map^;
        for Y:=0 to Bitmap.Height-1 do
        begin
          if Assigned(FileMappedBitmap) then Pix:=FileMappedBitmap.ScanLine[Y];
          SwapLine:=ImageLine;
          ImageLine:=LastLine;
          LastLine:=SwapLine;
          ...
          if (LineFilter<0) or (LineFilter=slfAverage) then AverageFilter(FilterLine[3]); // Average
...
var
  ...
  IsMNG : Boolean;
begin
  Assert(Ext=UpperCase(Ext),'Ext must be upper case');
  IsMNG:=Ext='MNG';
  ...
  try
    ...
    begin
      ...
      else if Assigned(AlphaChannel) then
      begin
        if (AlphaChannel.PixelFormat<>pf8bit) or
           (AlphaChannel.Width<>Bitmap.Width) or
           (AlphaChannel.Height<>Bitmap.Height) then
          raise Exception.Create(rsInvalidAlphaChannelFormat);
        IHDR.ColorType:=4; // Grayscale with alpha
        Planes:=2;
      end
      else
      begin
    ...
    begin
      ...
      begin
        if (AlphaChannel.PixelFormat<>pf8bit) or
           ...
           (AlphaChannel.Height<>Bitmap.Height) then
          raise Exception.Create(rsInvalidAlphaChannelFormat);
        IHDR.ColorType:=6; // 24 bit with alpha
        Planes:=4;
      end
      else
      begin
