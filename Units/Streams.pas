...
uses
  {$IFDEF CLR} Borland.Win32.Windows, Borland.Win32.WinUtils, Borland.Delphi.SysUtils, {$ELSE} Windows, SysUtils, {$ENDIF}
  Monitor;
  ...
  TFileStream =
    class (TSeekableStream)
      ...
      public
        ...
        property FileName: string read Name; // Name of file
        function Write(var Buf; Count: integer): integer; override;
        function Read(var Buf; Count: integer): integer; override;
...
begin
 ...
 N:=Next;
 {$IFDEF CLR} Free; {$ELSE} Destroy; {$ENDIF}
...
function TSeekableStream.Available: integer;
begin
...
begin
  ...
  {$IFDEF CLR}
  Handle:= CreateFile(Name, access, shared, nil, create, FILE_ATTRIBUTE_NORMAL, 0);
  {$ELSE}
...
begin
  {$IFDEF CLR}
  if not WriteFile( Handle, TBytes(Buf), Count, written, nil ) then
  {$ELSE}
...
begin
  {$IFDEF CLR}
  if not ReadFile( Handle, TBytes(Buf), Count, readb, nil ) then
  {$ELSE}
...
begin
  ...
  if Result<0 then raise EInOutError.CreateFmt( rs_IOError, [ GetLastErrorText ] );
end;
...
var
  SizeHi : DWord;
begin
  Result:= GetFileSize(Handle, @SizeHi);
  if (Result<0) or (SizeHi<>0) then Result:=MaxInt;
end;
...
begin
  if not GetFileTime(Handle,nil,nil,@Result) then RaiseLastOSError;
end;

procedure TFileStream.SetLastWriteTime(const Time: TFileTime);
begin
  if not SetFileTime(Handle,nil,nil,@Time) then RaiseLastOSError;
end;
{$ENDIF}
